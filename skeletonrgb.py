...
im = cv2.imread("make.jpg", 0) # ภาพ .jpg ไว้เทส
...
while cv2.waitKey(1) < 0:
    ...
    ax[0].imshow(im , cmap=plt.cm.binary)
    ax[0].set_title('Binary Image')
    ax[0].axis('off')

    ax[1].imshow(frame, cmap=plt.cm.binary)
    ...
    print('Success')
    #cv2.imshow('Human Skeleton using OpenCV', frame) #ตัวเเสดงทั้งกล้องเเละ gui
